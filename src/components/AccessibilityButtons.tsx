    ...
    <div className="fixed top-20 right-3 z-50 flex flex-col gap-2">
      <div className="accessibility-buttons bg-background/95 border rounded-lg p-2 shadow-lg">
        <div className="flex items-center gap-1">
          ...
          <Button variant="outline" size="sm" onClick={decreaseFontSize} disabled={fontSize === 'normal'} title="Diminuir fonte" className="p-2 h-8 w-8">
            <Minus className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={increaseFontSize} disabled={fontSize === 'xlarge'} title="Aumentar fonte" className="p-2 h-8 w-8">
            <Plus className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={resetFontSize} title="Resetar fonte" className="p-2 h-8 w-8">
            <RotateCcw className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={toggleHighContrast} title={`${highContrast ? 'Desativar' : 'Ativar'} alto contraste`} className="p-2 h-8 w-8">
